...
var lastBorgPageNumber: Int = -1;

fun getBorgPage(pageNumber: Int): List<Quote> {
    if (pageNumber == 0) {
        return ArrayList()
    }
    return parsePage(pageNumber.toString())
}
...
fun parsePage(pageNumber: String): List<Quote> {
    try {
        ...
        lastBorgPageNumber = getPageNumber(document)
        println("real last page number: $lastBorgPageNumber")
...
fun parsePrevPage(): List<Quote>? {
    if (lastBorgPageNumber <= 0) {
        return null
    }
    println("fetching and parsing next page (${lastBorgPageNumber - 1})")
    return getBorgPage(lastBorgPageNumber - 1);
}

fun parseBorgPageFromTop(byRatingPage: Int): List<Quote> {
    try {
...
fun getBorgRandomPage(): List<Quote> {
    try {
